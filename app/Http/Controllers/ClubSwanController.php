    ...
    static public function kycStatic(array $data)
    {
        try {
            ...
            if ($response->status == 'success') {
                ...
                $sql = 'INSERT INTO custom_log (content, user_id, operation, controller, http_code, route, status) VALUES (?, ?, ?, ?, ?, ?, ?)';
                $values = [
                    $response->message . ' - PayLoad: ' . json_encode($data) . ' - Response: ' . json_encode($response),
                    0,
                    'KYC Status Check',
                    'ClubSwanController',
                    '200',
                ...
                // Execute a consulta usando prepared statement

...
class HomeAdminController extends Controller
{
    ...
    public function indexAdmin()
    {
        ...
        $totalComission = Banco::selectRaw('sum(price) as total')
            ->first();

        if ($totalComission->total > 0) {
            $totalComission = number_format($totalComission->total, 2, ',', '.');
        } else {
            $totalComission = 0;
        }
        ...
        $orderpackages = OrderPackage::select(
            'orders_package.id',
            'orders_package.created_at',
            'users.name',
            'users.login',
            DB::raw('SUM(orders_package.price) as total')
        )
            ->join('users', 'users.id', '=', 'orders_package.user_id')
            ->groupBy('orders_package.id', 'orders_package.created_at', 'users.name', 'users.login')
        ...
        return view('admin.homeAdmin', ['lava' => $lava])->with(compact('totalComission', 'users', 'usersCont', 'withdrawSum', 'commissionSum', 'orderpackages', 'lava', 'cards', 'countcard', 'poll', 'specialCommision', 'userpay'));
    }
}
